...
def perturb(embedding, frequencies, alpha):
    spectrum = np.fft.fft(embedding, axis=0)
    spectrum[frequencies, :] *= alpha
    return np.fft.ifft(spectrum, axis=0).real
...
if __name__ == "__main__":
    ...
    print(f"doing work on {start_seed} to {end_seed} exclusive")
    print(args)
    ...
    alpha = scales[args.index]

    model = get_model(args.init_seed, dim, p)
    init_mlp = fuse_mlp(model)
    init_embedding = model.embedding.cpu().detach().numpy()
    init_embedding = perturb(init_embedding, [25, 34], alpha)
    ...
    for seed in tqdm(range(start_seed, end_seed)):
        ...
        if args.fix_embedding: 
            model = get_model(seed, dim, p, embedding=init_embedding)
            train_id, test_id = get_train_test(seed, 0.8, p)
        elif args.fix_mlp: 
            print("fix_mlp")
            init_embed = torch.normal(0,1,size=(p, dim)).detach().numpy()
            set_mlp_and_embedding(model, init_mlp, init_embed)
        elif args.fix_dataset: 
            model = get_model(seed, dim, p)
        else: 
            model = get_model(seed, dim, p)
            train_id, test_id = get_train_test(seed, 0.8, p)
    
        data_path = f"{args.data_path}/dim_{args.dim}_init_seed_{args.init_seed}_vary_seed_{seed}_index_{args.index}_id_{id}"
        if not os.path.exists(data_path):
            os.makedirs(data_path)
        np.save(os.path.join(data_path, 'init_mlp.npy'), init_mlp.detach().numpy())
        np.save(os.path.join(data_path, 'init_params.npy'), fuse_parameters(model).detach().numpy())
